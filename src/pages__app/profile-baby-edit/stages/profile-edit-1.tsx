...
const ProfileEditOneStage = () => {
  ...
  const { setChart, chart } = useAnketsBabysitter();
  ...
  return (
    <div className={styles['profile-edit-stage']}>
      <div className={styles['profile-edit-stage__top']}>
        ...
        <span className={styles['profile-edit-stage__top-description']}>шаг 1/11</span>
      </div>

      <h2 className={styles['profile-edit-stage__title']}>Выберите подходящие вам дни и время работы</h2>

      <Calendar setitems={setChart} items={chart}/>
      ...
      <div className={styles['profile-edit-stage__bottom']}>
        ...
        <button onClick={() => stage.setStage('second')} className={styles['profile-edit-stage__btn-bottom']}>
          Продолжить
